...
export default function PropertyInfoPage() {
  ...
  if (status === 'error' || !data || !data.data) {
      ...
      <Row>
        ...
        <Col className="px-0" style={{ background: '#ebecf0' }}>
          ...
          >
            ...
            <Modal.Body>
              <h6 style={{ margin: 0 }}>
                Are you sure you want to delete the property {data?.data.address},{' '}
            ...
            <Modal.Footer>
              ...
              <Button
                variant="danger"
                onClick={() => {
                  if (typeof data.data !== 'string' && data.data.id) {
                    mutate(data.data.id);
                  }
                }}
              >
          ...
          <Container fluid>
            ...
            <Container fluid style={{ flexDirection: 'column', textAlign: 'left' }}>
              <h5 className="mb-3">
                ...
                <button
                  type="button"
                  className="mx-3 btn btn-primary btn-sm"
                  onClick={() => {
                    if (typeof data.data !== 'string' && data.data.id) {
                      navigate(`/property/edit/${data?.data.id}`);
                    }
                  }}
                >
              ...
              <p>
                Address: {data?.data.address}, {data?.data.city}, {data?.data.state}
              </p>

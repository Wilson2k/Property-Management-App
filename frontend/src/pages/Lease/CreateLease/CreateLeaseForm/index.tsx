  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    if (isLastStep) {
      ...
      if (data.startDate && data.endDate && data.monthlyRent) {
        ...
        // Call api here
        setLoading(true);
        const leaseInput: LeaseCreateContext = {
          ...data,
          startDate: start,
          endDate: end,
          months: months,
  ...
  return (
    <Col className="px-0" style={{ background: '#ebecf0' }}>
      <Container fluid style={{ maxWidth: '1000px' }}>
        ...
        >
          <div onSubmit={(event) => void onSubmit(event)}>
            <div className="progress">
              <div
                className="progress-bar"
                role="progressbar"
                style={{ width: `${(100 / steps.length) * currentStepIndex}%` }}
                aria-valuenow={currentStepIndex + 1 / steps.length}
                aria-valuemin={0}
                aria-valuemax={100}
              ></div>
            </div>
            ...
            {isLastStep && (
              ...
              >
                ...
                {!isLoading && (
                  <button
                    className="btn btn-outline-success btn-lg px-5 mt-4"
            ...
            {isLastStep &&
              (response === 422 ? (
                ...
                <div className="card bg-danger">Lease already exists</div>
              ) : <></> && response === 500 ? (
                <div className="card bg-danger">Error creating lease</div>
              ) : <></> && response === 200 ? (
                <div className="card bg-success">Successfully created lease</div>
              ) : (
                <></>
              ))}
          </div>
